...
**Importante:** os templates de perfil não trazem a seção "Trabalho de frontend — obrigatório". Depois de aplicar o template, copiar essa seção do `CLAUDE.md` original (a versão versionada no repo) pro arquivo novo, logo depois do bloco "Fluxo de trabalho". Sem ela, o sistema perde a régua de qualidade visual.
...
Se nenhum responder, instalar via skills.sh:

```bash
npx skills add microsoft/playwright-cli
```

Se o `npx skills` não estiver disponível, cair pro global:
...
Não travar o setup por causa disso. Registrar o que ficou faltando e avisar:

> "Não consegui instalar o <ferramenta> agora. O resto do MazyOS funciona
> normal — só o fluxo de site/interface que vai ficar sem a régua de
> qualidade até você rodar `<comando>`. Anotei em `_memoria/estrategia.md`."
